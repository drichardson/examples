...
static void
uint_count_print_action(const void *nodep, const VISIT which, const int depth)
{
	...
	switch (which)
	{
	case postorder: // postorder is more commonly known as inorder, but this
			// API is weird
	case leaf:
		printf("%u: %u\n", count->value, count->counter);
		break;
	case preorder:
	case endorder: // more commonly called postorder, but postorder in this
		// API means inorder.
		break;
	}
}
...
{
	unsigned int data_set[] = {5, 2, 9, 8, 3, 2, 2, 1, 7, 9, 1, 0,
	...
	for (int i = 0; i < sizeof(data_set) / sizeof(data_set[0]); ++i)
	{
		...
		uint_count **count =
		    tsearch(new_count, &root, uint_count_compare);
		if (new_count == *count)
		{
