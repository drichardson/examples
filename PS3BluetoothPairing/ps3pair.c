/*
 * ps3pair.c - based on sixpair.c version 2007-04-18
 * Compile with: gcc -o ps3pair ps3pair.c -lusb
 */
...
void show_master(usb_dev_handle *devh, int itfnum)
{
    ...
    unsigned char msg[8];
    int res = usb_control_msg(devh, USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE,
...
void set_master(usb_dev_handle *devh, int itfnum, int mac[6])
{
    ...
    int res = usb_control_msg(devh, USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE,
			      0x09,
			      0x03f5, itfnum, msg, sizeof(msg),
			      5000);
  
    if ( res < 0 )
	fatal("USB_REQ_SET_CONFIGURATION");
}
		    ...
		    struct usb_config_descriptor *cfg, int itfnum)
{
    ...
    if ( argc >= 2 )
    {
	int c = sscanf(argv[1], "%x:%x:%x:%x:%x:%x", &mac[0],&mac[1],&mac[2],&mac[3],&mac[4],&mac[5]);
