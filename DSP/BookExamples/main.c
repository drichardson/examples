...
static void int32_convolution()
{
	...
	Int32Signal *y =
	    Int32Signal_ConvolveUsingInputSideAlgorithm_AllocResult(x, h);
...
static void float32_convolution()
{
	...
	int i;
	for (i = 0; i < x->sample_count; ++i)
		x->samples[i] = sinf(2 * M_PI * i / 20.0) * 4 * i /
				((float)x->sample_count);

	h->samples[h->sample_count / 2] = 1.0; // A high-pass filter.

	// Float32Signal *y =
	// Float32Signal_ConvolveUsingInputSideAlgorithm_AllocResult(x, h);
	Float32Signal *y =
	    Float32Signal_ConvolveUsingOutputSideAlgorithm_AllocResult(x, h);
